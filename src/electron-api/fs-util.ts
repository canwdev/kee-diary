...
// 打开文件选择器
export function showFileChooser(config: any = {}) {
  return new Promise(async (resolve, reject) => {
    ...
    if (sizeLimit > 0) {
      ...
      if (allSize > sizeLimit) {
        throw new Error(
          `File size cannot exceed ${(sizeLimit / 1024).toFixed(2)} KB, current size: ${(
            allSize / 1024
          ).toFixed(2)} KB`
        )
      }
...
export function showSaveDialog(text: string, options: any = {}) {
  return new Promise(async (resolve, reject) => {
    const {title, defaultPath, buttonLabel, filters} = options
...
export function readFileAsPlainText(path: string) {
